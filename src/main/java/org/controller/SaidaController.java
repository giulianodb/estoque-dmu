...
public class SaidaController implements Serializable  {
	...
	public String redirecionarPaginaEditarSaida(String codLote) {
		
		return "/pages/movimentacao/saida/editar_saida?faces-redirect=true&codLote="+codLote;
	 ...
	 */
	public void iniciarEdicaoMovimentacaoSaida(String codLote) throws ApplicationException {
		if (codLote == null || codLote.equals("")) {
			iniciarIncluirMovimentacaoSaida();
		}
		else {
			LoteMovimentacao lote = loteService.obterLoteMovimentacaoByChave(Integer.decode(codLote));
			iniciarAlterarMovimentacaoSaida(lote);
		}
		
	}
	
	
	public void iniciarIncluirMovimentacaoSaida() throws ApplicationException{
		...
		listaTipoParceiroCombo.remove(TipoParceiroEnum.ANONIMO);
	...
	public void iniciarAlterarMovimentacaoSaida(LoteMovimentacao lote) throws ApplicationException{
...
//		return "/pages/movimentacao/saida/editar_saida";
	...
	public String incluirSaida() throws ApplicationException{
		...
		for (Movimentacao saida : listaMovimentacao) {
			System.out.println("==========Movimentação de entrada cadastrada. Código: " + saida.getId() + " Lote: "+saida.getLoteMovimentacao().getCodigo() + " Data Mov cadastrada: " + saida.getLoteMovimentacao().getData() + " Data ação: " +new Date() );
		}
	...
	public void adicionarSaidaLote() throws ApplicationException{
		if (saida.getQuantidade().equals(0.0f)) {
		...
		for (Movimentacao mov : listaMovimentacao) {
			if (mov.getProduto().getId().equals(saida.getProduto().getId())) {
...
//		prepararIncluirSaida();
	...
	public void prepararIncluirSaida() throws ApplicationException {
		...
		saida.getLoteMovimentacao().setData(configApp.getDataUltimaMovimentacao());
		
		saidaDadosReceptor.getLoteMovimentacao().setData(configApp.getDataUltimaMovimentacao());
